...
alter table reservations
add constraint reservations_priority_type_check
  check (priority_type is null or priority_type in ('PWD', 'Pregnant', 'Senior Citizen'));

alter table reservations
add column if not exists is_priority boolean generated always as (priority_type is not null) stored;

create index if not exists idx_reservations_priority_dispatch
  on reservations(queue_date, status, window_id, is_priority desc, created_at asc);
